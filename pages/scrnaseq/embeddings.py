...
def scdata_embedding_page():
    ...
    embeds = available_embeddings(scdata)
    if not embeds:
    ...
    with c2:
        ...
        if color_mode == "Metadata column":
            obs_color_col = st.selectbox("Metadata column (obs)", options=["(none)"] + list(scdata.obs.columns), index=0)
        else:
    ...
    if subset_available and sub_col != "(none)":
        if len(subset_values) == 0:
            scdata_view = scdata[[]]  # empty view (keeps app running)
        else:
    ...
    color_arg = resolve_color(scdata, color_mode, obs_color_col, gene_text)
    ...
    if scdata_view.n_obs == 0:
        ...
        render_plot(scdata_view, plot_type, color_arg=color_arg, size=pt_size, fig_w=curr_w, fig_h=curr_h,
